...
import { Button, Checkbox, InputNumber, Tag, Space } from 'antd'
    ...
    return (
        <div>
            <div className={sh.mongoToolbar}>
                <Button type="primary" size="small" icon={<Play size={13} />} disabled={busy} onClick={run}>
                    运行聚合
                </Button>
                <Button size="small" disabled={busy} onClick={explainPipeline}>
                    执行计划
                </Button>
                <Checkbox checked={allowDisk} onChange={e => setAllowDisk(e.target.checked)}>
                    允许磁盘使用
                </Checkbox>
                <Space size={6} className={sh.mongoDim}>
                    <span>超时(ms):</span>
                    <InputNumber size="small" style={{ width: 90 }} min={0} value={maxTime} onChange={v => setMaxTime(v ?? 0)} />
                </Space>
                <span className={sh.toolbarRight}>
                    {error && <span className={sh.errorText}>{error}</span>}
                    <Tag color="geekblue">结果 {count} 条 · {duration}ms</Tag>
                </span>
            </div>

            <label className={sh.mongoField} style={{ marginTop: 8 }}>
                <span className={sh.mongoDim}>聚合管道（JSON 数组，每个元素是一个阶段）</span>
                <CodeEditor lang="json" height="300px" value={pipeline} onChange={setPipeline} />
            </label>

            {result.length > 0 && (
                <>
                    <div className={sh.mongoH} style={{ marginTop: 12 }}>结果</div>
            ...
            {explain && (
                <>
                    <div className={sh.mongoH} style={{ marginTop: 12 }}>执行计划</div>
