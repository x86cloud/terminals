    ...
    return (
        <div>
            <div className={sh.mongoToolbar} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px' }}>
                <Button type="primary" size="small" icon={<RotateCw size={13} />} disabled={busy} onClick={load}>
                    刷新监控
                </Button>
                <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {health && (
                        health.ok ? (
                            <Tag color="success">健康 · 延迟 {health.latencyMs}ms</Tag>
                        ) : (
                            <Tag color="error">异常 · {health.error}</Tag>
                        )
                    )}
                </span>
            </div>
            ...
            {health && (
                <>
                    <div className={sh.mongoH} style={{ marginTop: 12 }}>健康检查</div>
                    ...
                    {health.hosts && health.hosts.length > 0 && (
                        <div className={sh.mongoRow} style={{ flexWrap: 'wrap', marginBottom: 6, gap: 6 }}>
                            {health.hosts.map((h) => (
