...
import CreateKeyModal, { RedisKeyType } from '@/pages/redis/CreateKeyModal'
...
export default function RedisClient({ session, onClose, onDbChange }: Props) {
    ...
    const keyTree = useMemo(() => buildKeyTree(data.keys, ':'), [data.keys])
    ...
    const flash = useCallback((m: string) => {
        if (!m) return
        if (m.includes('失败') || m.includes('ERROR') || m.includes('错误')) {
            message.error(m)
        } else if (
            m.includes('成功') ||
            m.includes('已保存') ||
            ...
            message.success(m)
        } else if (m.includes('不是合法') || m.includes('冲突') || m.includes('警告') || m.includes('中止')) {
            message.warning(m)
        } else {
            message.info(m)
        }
    ...
    const switchDb = async (target?: number) => {
        const n = typeof target === 'number' ? target : (Number(dbInput) || 0)
        if (n === db) return
        try {
    ...
    const handleCreateKey = async (
        ...
        await API.redisSet(session.id, newKey, keyType, serializedValue, keyTtl)
        flash(`已成功创建键 [${newKey}]`)
        await loadKeys(true)
        await loadValue(newKey)
    }
        ...
        <div className={r.redisPane}>
            ...
            {tab === 'keys' && (
                <KeysTab
                    ...
                    runRaw={runRaw}
                    flash={flash}
                    onOpenCreateKey={() => setCreateKeyOpen(true)}
                />
            )}
