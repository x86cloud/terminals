...
func (a *App) MongoParseURI(uri string) (MongoURIInfo, error) {
	return parseMongoURI(uri)
}

// buildMongoURI 在未提供完整连接串时，用离散字段拼装连接串。
func buildMongoURI(cfg ServerConfig) string {
...
func buildMongoTLS(cfg ServerConfig) (*tls.Config, error) {
...
func buildMongoOptions(cfg ServerConfig) (*options.ClientOptions, error) {
...
func (m *mongoManager) open(id string, cfg ServerConfig) error {
...
func (a *App) mustMongo(id string) (*mongoClient, error) {
	mc, ok := a.mongoMgr.get(id)
	if !ok {
...
// MongoClose 关闭连接。
func (a *App) MongoClose(id string) {
	a.mongoMgr.close(id)
}

// MongoTestConnection 使用临时配置测试连通性，不写入连接池。
func (a *App) MongoTestConnection(cfg ServerConfig) (map[string]any, error) {
	opts, err := buildMongoOptions(cfg)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoCollections(id, db string) ([]map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoCreateDatabase(id, db, firstCollection string) error {
	if strings.TrimSpace(firstCollection) == "" {
		firstCollection = "default"
	}
	return a.MongoCreateCollection(id, db, firstCollection)
}

// MongoDropDatabase 删除数据库。
func (a *App) MongoDropDatabase(id, db string) error {
	mc, err := a.mustMongo(id)
	if err != nil {
		return err
	}
...
func (a *App) MongoCreateCollection(id, db, coll string) error {
	mc, err := a.mustMongo(id)
	if err != nil {
		return err
	}
...
func (a *App) MongoDropCollection(id, db, coll string) error {
	mc, err := a.mustMongo(id)
	if err != nil {
		return err
	}
...
func (a *App) MongoRenameCollection(id, db, coll, newName string) error {
	mc, err := a.mustMongo(id)
	if err != nil {
		return err
	}
...
func (a *App) MongoCollectionStats(id, db, coll string) (map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoInferSchema(id, db, coll string, sampleSize int) ([]map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoGetValidator(id, db, coll string) (map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoSetValidator(id, db, coll, validatorJSON, level, action string) error {
	mc, err := a.mustMongo(id)
	if err != nil {
		return err
	}
...
func (a *App) MongoValidateDocument(id, db, coll, docJSON string) (map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoFind(id string, spec MongoQuerySpec) (MongoFindResult, error) {
...
func (a *App) MongoCountDocuments(id, db, coll, filterJSON string) (int64, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return 0, err
	}
...
func (a *App) MongoDistinct(id, db, coll, field, filterJSON string) ([]string, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoExplain(id string, spec MongoQuerySpec, verbosity string) (string, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return "", err
	}
...
func (a *App) MongoInsertOne(id, db, coll, docJSON string) (string, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return "", err
	}
...
func (a *App) MongoInsertMany(id, db, coll, docsJSON string, ordered bool) (map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoUpdateOne(id, db, coll, filterJSON, updateJSON string, upsert bool) (map[string]any, error) {
	return a.mongoUpdate(id, db, coll, filterJSON, updateJSON, upsert, false)
}

// MongoUpdateMany 批量更新。
func (a *App) MongoUpdateMany(id, db, coll, filterJSON, updateJSON string, upsert bool) (map[string]any, error) {
	return a.mongoUpdate(id, db, coll, filterJSON, updateJSON, upsert, true)
}

func (a *App) mongoUpdate(id, db, coll, filterJSON, updateJSON string, upsert, many bool) (map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoReplaceOne(id, db, coll, filterJSON, docJSON string, upsert bool) (map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoDeleteOne(id, db, coll, filterJSON string) (int64, error) {
	return a.mongoDelete(id, db, coll, filterJSON, false)
}

// MongoDeleteMany 批量删除。
func (a *App) MongoDeleteMany(id, db, coll, filterJSON string) (int64, error) {
	return a.mongoDelete(id, db, coll, filterJSON, true)
}

func (a *App) mongoDelete(id, db, coll, filterJSON string, many bool) (int64, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return 0, err
	}
...
func (a *App) MongoFindOneAndUpdate(id, db, coll, filterJSON, updateJSON string, returnNew bool) (string, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return "", err
	}
...
func (a *App) MongoBulkWrite(id, db, coll string, ops []MongoBulkOp, ordered bool) (map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoAggregate(id, db, coll, pipelineJSON string, allowDiskUse bool, maxTimeMS int) (MongoFindResult, error) {
	out := MongoFindResult{Documents: []string{}}
	mc, err := a.mustMongo(id)
	if err != nil {
		return out, err
...
func (a *App) MongoAggregateExplain(id, db, coll, pipelineJSON string) (string, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return "", err
	}
...
func (a *App) MongoRunCommand(id, db, commandJSON string) (string, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return "", err
	}
...
// MongoIndexes 列出集合索引。
func (a *App) MongoIndexes(id, db, coll string) ([]map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoCreateIndex(id, db, coll, keysJSON, name string, unique, sparse bool, expireAfterSeconds int) (string, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return "", err
	}
...
// MongoDropIndex 删除索引。
func (a *App) MongoDropIndex(id, db, coll, name string) error {
	mc, err := a.mustMongo(id)
	if err != nil {
		return err
	}
...
func (a *App) MongoIndexStats(id, db, coll string) ([]string, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoTransaction(id string, ops []MongoTxOp) (map[string]any, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return nil, err
	}
...
func (a *App) MongoWatch(id, scope, db, coll, pipelineJSON, fullDocument string) (string, error) {
	mc, err := a.mustMongo(id)
	if err != nil {
		return "", err
	}
	...
	go func() {
		...
		for stream.Next(ctx) {
			...
			wruntime.EventsEmit(a.ctx, "mongo:change:"+id, map[string]any{
				"watchKey":  key,
