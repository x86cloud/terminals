...
func (m *SqliteManager) SetContext(ctx context.Context) {
	m.ctx = ctx
}
...
func (m *SqliteManager) SqliteOpenFile() (string, error) {
	return wruntime.OpenFileDialog(m.ctx, wruntime.OpenDialogOptions{
...
func (m *SqliteManager) SqliteConnect(id, path string) (bool, error) {
	if err := m.Open(id, path); err != nil {
		return false, err
	}
	return true, nil
}
...
func (m *SqliteManager) SqliteClose(id string) {
	m.Close(id)
}

// SqliteInfo 返回当前连接的基本信息：文件路径与大小。
func (m *SqliteManager) SqliteInfo(id string) (map[string]any, error) {
	mc, ok := m.Get(id)
	if !ok {
...
func (m *SqliteManager) SqliteTables(id string) ([]map[string]any, error) {
	mc, ok := m.Get(id)
	if !ok {
		return nil, errors.New("SQLite 连接不存在或已断开")
	}
...
func (m *SqliteManager) SqliteDescribe(id, table string) ([]map[string]any, error) {
	mc, ok := m.Get(id)
	if !ok {
		return nil, errors.New("SQLite 连接不存在或已断开")
	}
...
func (m *SqliteManager) SqliteSelect(id, table string, limit, offset int) (map[string]any, error) {
	...
	cols, rows, err := m.querySqlite(id, sqlText)
	if err != nil {
		return nil, err
	}
...
// SqliteCount 返回表总行数。
func (m *SqliteManager) SqliteCount(id, table string) (int64, error) {
	cols, rows, err := m.querySqlite(id, fmt.Sprintf("SELECT COUNT(*) AS cnt FROM %s", quoteSqliteIdent(table)))
	if err != nil {
		return 0, err
	}
...
func (m *SqliteManager) SqliteIndexes(id, table string) ([]map[string]any, error) {
	mc, ok := m.Get(id)
	if !ok {
		return nil, errors.New("SQLite 连接不存在或已断开")
	}
...
func (m *SqliteManager) SqliteRun(id, sqlText string) (map[string]any, error) {
	mc, ok := m.Get(id)
	if !ok {
		return nil, errors.New("SQLite 连接不存在或已断开")
	}
	...
	if sqliteIsReadQuery(trimmed) && !strings.HasPrefix(strings.ToUpper(trimmed), "PRAGMA") {
		cols, rows, err := m.querySqlite(id, trimmed)
		if err != nil {
			return nil, err
		}
...
func (m *SqliteManager) SqliteSchema(id string) (map[string]any, error) {
	tables, err := m.SqliteTables(id)
	if err != nil {
		return nil, err
	}
	type col struct {
	...
	for _, t := range tables {
		...
		cols, e := m.SqliteDescribe(id, name)
		if e != nil {
			continue
		}
...
// SqliteInsert 插入一行记录
func (m *SqliteManager) SqliteInsert(id, table string, columns []string, values []any) (int64, error) {
	mc, ok := m.Get(id)
	if !ok {
		return 0, errors.New("SQLite 连接不存在或已断开")
	}
	...
	sqlText := fmt.Sprintf("INSERT INTO %s (%s) VALUES (%s)",
		quoteSqliteIdent(table), strings.Join(quotedCols, ","), strings.Join(placeholders, ","))
...
// SqliteUpdate 更新单行或多行数据
func (m *SqliteManager) SqliteUpdate(id, table string, setCols []string, setVals []any, whereCols []string, whereVals []any) (int64, error) {
	mc, ok := m.Get(id)
	if !ok {
		return 0, errors.New("SQLite 连接不存在或已断开")
	}
	...
	for i, col := range setCols {
		setParts[i] = quoteSqliteIdent(col) + " = ?"
	}
	args := append([]any{}, setVals...)
	whereParts := make([]string, len(whereCols))
	for i, col := range whereCols {
		whereParts[i] = quoteSqliteIdent(col) + " = ?"
		args = append(args, whereVals[i])
	}
	sqlText := fmt.Sprintf("UPDATE %s SET %s", quoteSqliteIdent(table), strings.Join(setParts, ", "))
	if len(whereParts) > 0 {
...
// SqliteDelete 删除指定条件的行
func (m *SqliteManager) SqliteDelete(id, table string, whereCols []string, whereVals []any) (int64, error) {
	mc, ok := m.Get(id)
	if !ok {
		return 0, errors.New("SQLite 连接不存在或已断开")
	}
	...
	sqlText := fmt.Sprintf("DELETE FROM %s WHERE %s", quoteSqliteIdent(table), strings.Join(whereParts, " AND "))
...
// SqliteCreateTable 新建数据表
func (m *SqliteManager) SqliteCreateTable(id, table, defs string) error {
	...
	_, err := mc.db.Exec(fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s (%s)", quoteSqliteIdent(table), defs))
	return err
}

// SqliteDropTable 删除数据表
func (m *SqliteManager) SqliteDropTable(id, table string) error {
	mc, ok := m.Get(id)
	if !ok {
		return errors.New("SQLite 连接不存在或已断开")
	}
	_, err := mc.db.Exec("DROP TABLE IF EXISTS " + quoteSqliteIdent(table))
	return err
}
...
func (m *SqliteManager) SqliteCreateIndex(id, table, name, colsCSV string, unique bool) error {
	...
	if len(cols) == 0 {
		return errors.New("请指定索引列")
	}
	for i := range cols {
		cols[i] = quoteSqliteIdent(strings.TrimSpace(cols[i]))
	}
	kind := ""
	if unique {
		kind = "UNIQUE "
	}
	sqlText := fmt.Sprintf("CREATE %sINDEX IF NOT EXISTS %s ON %s (%s)", kind, quoteSqliteIdent(name), quoteSqliteIdent(table), strings.Join(cols, ","))
	_, err := mc.db.Exec(sqlText)
	return err
}

// SqliteDropIndex 删除索引
func (m *SqliteManager) SqliteDropIndex(id, table, name string) error {
	mc, ok := m.Get(id)
	if !ok {
		return errors.New("SQLite 连接不存在或已断开")
	}
	_, err := mc.db.Exec("DROP INDEX IF EXISTS " + quoteSqliteIdent(name))
	return err
}
