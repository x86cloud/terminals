...
// TestConnection 测试 MySQL 数据库连通性并返回网络延迟
func (m *MysqlManagerEx) TestConnection(cfg core.ServerConfig) (map[string]any, error) {
	dsn, tunnel, err := buildMysqlDSNExtended(cfg)
	if err != nil {
		return nil, err
	}
...
func (m *MysqlManagerEx) MysqlGrants(id, user, host string) (string, error) {
	...
	rows, err := queryRaw(mc.db, fmt.Sprintf("SHOW GRANTS FOR %s", quoteUserHost(user, host)))
	if err != nil {
		return "", err
	}
...
func (m *MysqlManagerEx) MysqlCreateUser(id, user, host, password, authPlugin string, lock bool) error {
	mc, ok := m.Get(id)
	if !ok {
		return errors.New("MySQL 连接不存在或已断开")
	}
	...
	sqlText := fmt.Sprintf("CREATE USER %s", quoteUserHost(user, host))
	if password != "" {
		if authPlugin != "" {
			sqlText += fmt.Sprintf(" IDENTIFIED WITH %s BY %s", quoteLiteral(authPlugin), quoteLiteral(password))
		} else {
			sqlText += fmt.Sprintf(" IDENTIFIED BY %s", quoteLiteral(password))
		}
	}
	if lock {
		sqlText += " ACCOUNT LOCK"
	}

	if _, err := mc.db.Exec(sqlText); err != nil {
		return err
	}
	_, _ = mc.db.Exec("FLUSH PRIVILEGES")
	return nil
}

func (m *MysqlManagerEx) MysqlDropUser(id, user, host string) error {
	mc, ok := m.Get(id)
	if !ok {
		return errors.New("MySQL 连接不存在或已断开")
	}
	if strings.ToLower(strings.TrimSpace(user)) == "root" {
		return errors.New("系统安全保护: 禁止对 root 超级管理员账户执行删除操作")
	}
	sqlText := fmt.Sprintf("DROP USER %s", quoteUserHost(user, host))
	if _, err := mc.db.Exec(sqlText); err != nil {
		return err
	}
	_, _ = mc.db.Exec("FLUSH PRIVILEGES")
...
func (m *MysqlManagerEx) MysqlChangeUserPassword(id, user, host, newPassword string) error {
	mc, ok := m.Get(id)
	if !ok {
		return errors.New("MySQL 连接不存在或已断开")
	}
	sqlText := fmt.Sprintf("ALTER USER %s IDENTIFIED BY %s", quoteUserHost(user, host), quoteLiteral(newPassword))
	if _, err := mc.db.Exec(sqlText); err != nil {
		return err
	}
	_, _ = mc.db.Exec("FLUSH PRIVILEGES")
...
func (m *MysqlManagerEx) MysqlToggleUserLock(id, user, host string, lock bool) error {
	mc, ok := m.Get(id)
	if !ok {
		return errors.New("MySQL 连接不存在或已断开")
	}
	if lock && strings.ToLower(strings.TrimSpace(user)) == "root" {
		return errors.New("系统安全保护: 禁止锁定 root 超级管理员账户")
	}
	action := "UNLOCK"
	if lock {
		action = "LOCK"
	}
	sqlText := fmt.Sprintf("ALTER USER %s ACCOUNT %s", quoteUserHost(user, host), action)
	if _, err := mc.db.Exec(sqlText); err != nil {
		return err
	}
	_, _ = mc.db.Exec("FLUSH PRIVILEGES")
...
func (m *MysqlManagerEx) MysqlGrantPrivileges(id, user, host, dbName, table string, privs []string, withGrantOption bool) error {
	mc, ok := m.Get(id)
	if !ok {
		return errors.New("MySQL 连接不存在或已断开")
	}
	...
	if dbName != "" && dbName != "*" {
		if table != "" && table != "*" {
			target = fmt.Sprintf("%s.%s", quoteIdent(dbName), quoteIdent(table))
		} else {
			target = fmt.Sprintf("%s.*", quoteIdent(dbName))
		}
	}

	sqlText := fmt.Sprintf("GRANT %s ON %s TO %s", privStr, target, quoteUserHost(user, host))
	if withGrantOption {
...
func (m *MysqlManagerEx) MysqlRevokePrivileges(id, user, host, dbName, table string, privs []string) error {
	mc, ok := m.Get(id)
	if !ok {
		return errors.New("MySQL 连接不存在或已断开")
	}
	if strings.ToLower(strings.TrimSpace(user)) == "root" {
		return errors.New("系统安全保护: 禁止撤销 root 超级管理员权限")
	}
	if len(privs) == 0 {
		return errors.New("至少选择一项要撤销的权限")
	}
	privStr := strings.Join(privs, ", ")
	target := "*.*"
	dbName = strings.TrimSpace(dbName)
	...
	sqlText := fmt.Sprintf("REVOKE %s ON %s FROM %s", privStr, target, quoteUserHost(user, host))
	if _, err := mc.db.Exec(sqlText); err != nil {
		return err
	}
	_, _ = mc.db.Exec("FLUSH PRIVILEGES")
...
func (m *MysqlManagerEx) MysqlRevokeAllPrivileges(id, user, host string) error {
	mc, ok := m.Get(id)
	if !ok {
		return errors.New("MySQL 连接不存在或已断开")
	}
	...
	sqlText := fmt.Sprintf("REVOKE ALL PRIVILEGES, GRANT OPTION FROM %s", quoteUserHost(user, host))
	if _, err := mc.db.Exec(sqlText); err != nil {
		return err
	}
	_, _ = mc.db.Exec("FLUSH PRIVILEGES")
...
func (m *MysqlManagerEx) MysqlExportToFileEx(id, db, mode, source, table, sqlText string, limit int) (string, error) {
	...
	path, err := core.SaveFileDialog("导出数据", base+"."+ext)
	if err != nil {
		return "", err
	}
	if path == "" {
...
func (m *MysqlManagerEx) MysqlBackupToFile(id, db string) (string, error) {
	...
	path, err := core.SaveFileDialog("备份数据库", db+"_backup.sql")
	if err != nil {
		return "", err
	}
	if path == "" {
