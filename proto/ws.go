...
func (m *WsManager) WsConnect(req WsConnectRequest) (WsConnectResult, error) {
	...
	core.EmitEvent("ws:status:"+id, WsStatusEvent{Status: "open"})
...
func (m *WsManager) wsReadLoop(conn *wsConn) {
	for {
		...
		if err != nil {
			core.EmitEvent("ws:status:"+conn.id, WsStatusEvent{Status: "closed", Error: err.Error()})
			core.EmitEvent("ws:message:"+conn.id, WsMessageEvent{
...
func (m *WsManager) WsClose(id string) {
	...
	core.EmitEvent("ws:status:"+id, WsStatusEvent{Status: "closed"})
}
