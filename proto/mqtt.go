...
func (m *MqttManager) Connect(cfg core.ServerConfig) (bool, error) {
	...
	opts := pahomqtt.NewClientOptions().
		...
		SetDefaultPublishHandler(func(_ pahomqtt.Client, msg pahomqtt.Message) {
			if m.ctx != nil {
				wruntime.EventsEmit(m.ctx, "mqtt:message:"+id, map[string]any{
					"topic":    msg.Topic(),
		...
		SetOnConnectHandler(func(_ pahomqtt.Client) {
			if m.ctx != nil {
				wruntime.EventsEmit(m.ctx, "mqtt:status:"+id, map[string]any{"connected": true})
			}
		}).
		SetConnectionLostHandler(func(_ pahomqtt.Client, err error) {
			if m.ctx != nil {
				wruntime.EventsEmit(m.ctx, "mqtt:status:"+id, map[string]any{"connected": false, "error": err.Error()})
			}
		})
